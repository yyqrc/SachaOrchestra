...
- 当前 release：`1.0.8`；发布身份以同名 annotated tag `v1.0.8` 为准。
- 当前源码版本：`1.0.8`，本次确定范围无剩余待发布改动。
- 当前 release Scope：独立委派单元默认新建代理，仅同一子任务的连续工作且上下文仍有效时复用；Codex 明确历史传输与容量边界。DSH 伴侣源码随仓库提交，包含 PTC 工具过滤修复、worker 过滤配置修复及可用能力分组浮层；伴侣不单独发布包或安装。沿用既有流程，不新增 Gate 或状态格式。
