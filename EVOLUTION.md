...
- 当前 release：`1.0.5`；发布身份以同名 annotated tag `v1.0.5` 为准。
- 当前源码版本：`1.0.5`，本次确定范围无剩余待发布改动。
- 当前 release Scope：减少无新信息的等待/状态查询，明确派发后的调查责任、串行实施执行者与有效内容复用；保持既有角色、模型路由、授权和单写入者边界。不新增 Gate 或状态格式，独立 DSH 配套包不随主插件发布或安装。
