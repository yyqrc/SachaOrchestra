...
- 当前 release：`1.0.2`；发布身份以同名 annotated tag `v1.0.2` 为准。
- 当前源码版本：`1.0.2`，本次确定范围无剩余待发布改动。
- 当前 release Scope：关键决定明确的任务直接执行，局部澄清和 Agent 自行提议 Spec 不触发正式规划；有界工作优先完整派发，共享可变环境由一个执行者集中操作并合并同版本验证；Codex 自动路线使用 Astra low、Sol medium 和 Luna，规划高档不自动延续到实施。沿用既有 v2、授权、证据复用及恢复边界；独立 DSH 配套包不随主插件发布或安装。
