...
- 当前 release：`1.0.9`；发布身份以同名 annotated tag `v1.0.9` 为准。
- 当前源码版本：`1.0.9`，本次确定范围无剩余待发布改动。
- 当前 release Scope：状态问答后继续已授权执行；协调通信以改变下一步的事件为准，Codex 按活性核对点有界查询；研究及时收敛并复用有效证据。精简协调合同，补齐顶层协调与任务转移细化图，更新连续执行和代理复用场景输入与裁决要求。沿用既有流程，不新增 Gate 或状态格式。
