...
- 当前成熟度：`0.14.2` 的通用模型路由 source-scenario 由主任务和独立 Reviewer 两次验证器运行通过，实际子任务遥测分别为 Luna Max 与 Sol Medium，独立结论为 `pass`。快速发版跳过普通回归、Skill/Plugin validator、完整 release coherence、安装/cache parity 和 fresh discovery；安装后 Runtime 行为未验证。
