...
- 当前 release：`1.0.1`；发布身份以同名 annotated tag `v1.0.1` 为准。
- 当前源码版本：`1.0.1`，本次确定范围无剩余待发布改动。
- 当前 release Scope：主动识别当前目标及原型设计的入口候选并提出选择；等待期间继续已授权且不依赖决定的事实调查；提问方式适配当前宿主用途限制；派发携带可复用事实、证据版本与缺口；普通只读工具失败可核对等价能力，保留接口身份验证与写入恢复边界。沿用既有 Astra、Codex v2、生成器和 Spec 收口边界；独立 DSH 配套包不随主插件发布或安装。
...
- 本次入口场景的实际读取与可见行为经独立评估符合预期；原生派发控制消息不可读，完整隔离与安装后发现可靠性仍未证明。合并后的独立调查与宿主映射完成源码核对；补充运行场景因宿主代理数量限制未执行，不作对应运行通过声明。
