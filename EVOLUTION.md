...
- 当前 release：`1.0.11`；发布身份以同名 annotated tag `v1.0.11` 为准。
- 当前源码版本：`1.0.11`，本次确定范围无剩余待发布改动。
- 当前 release Scope：从方案讨论、Spec 生成、实施到 Review 核对改动必要性，限制无依据的机制与验证设施扩张；明确数据迁移由风险告知和用户决定驱动。Codex 主任务的型号要求不默认扩展到子代理。沿用既有流程，不新增 Gate 或状态格式。
