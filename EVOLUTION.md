...
- 当前 release：`1.0.7`；发布身份以同名 annotated tag `v1.0.7` 为准。
- 当前源码版本：`1.0.7`，本次确定范围无剩余待发布改动。
- 当前 release Scope：继续已有目标时核对并恢复已接受的 Sacha；诊断转为方案或实质方案变动时必须重新判断入口，满足条件时必须询问。Spec 区分需求与可替换实现，最新明确决定替换冲突旧内容；停用与回退约束按需记录，不默认新增失败恢复或状态机。沿用既有流程，不新增 Gate 或状态格式；独立 DSH 配套包不随主插件发布或安装。
