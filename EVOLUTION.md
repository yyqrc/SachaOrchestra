...
- 当前 release：`1.0.10`；发布身份以同名 annotated tag `v1.0.10` 为准。
- 当前源码版本：`1.0.10`，本次确定范围无剩余待发布改动。
- 当前 release Scope：加强 Spec 自包含交付：Planner 仅凭落盘 Spec、项目规则和明确引用的资料推演实施与验收，区分遗漏决定、可查事实、实质未决项与局部实现自由；核对不通过时原位修订。明确 Spec 技术方案与既有执行状态记录的分工。沿用既有流程，不新增 Gate 或状态格式。
