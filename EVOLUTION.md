...
- 当前 release：`1.0.12`；发布身份以同名 annotated tag `v1.0.12` 为准。
- 当前源码版本：`1.0.12`，本次确定范围无剩余待发布改动。
- 当前 release Scope：Spec 必须完成必要设计并落实到精确代码位置、接口和实施步骤，执行者只处理允许的机械适配，设计缺口交回主任务；合并重复规格要求与模板内容，收紧探索、规划和评审中的无依据扩张。Codex 执行代理及派发入口显式衔接 Spec 标准。沿用既有流程，不新增 Gate 或状态格式。
