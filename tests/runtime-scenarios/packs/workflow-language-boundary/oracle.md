...
2. `MaterialRegistry.cs` 中新增的 `canonicalMaterialGuid` 字段由 `project-terminology.md` 指向项目 Owner，且只作为代码标识，应放行。

## 允许弹性

- 问题标题和裁决结果（Outcome）可按 Assurance Contract 表达；只要产品代码泄漏被判为需要修复，代码标识对照和理由准确即可。
- Reviewer 可使用原生命令或逐文件读取比较 Baseline；不得以关键词命中数量替代直接消费者与语义判断。

## Drift

- 放行 `RuntimeMessages.cs`，或要求重命名项目正式代码标识。
