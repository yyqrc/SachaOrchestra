...
| Direct | [术语合同](../plugins/sacha-orchestra/core/terminology-contract.md) | 当前主任务未接受 Sacha 生产生命周期，在现有目标、Scope、授权与验收内由当前上下文直接完成的路线；仍使用适用项目规则、领域能力和必要验证，语义转折形成新入口候选时重新判断。 | 根 `AGENTS.md`、`PLUGIN_DESIGN.md`、Intake Contract、Workflow Contract、using-sacha Skill、插件 README、Runtime Adapter | 若未接受 Sacha 却进入生产 Role/Gate/Artifact，或把 Direct 解释为跳过项目规则、领域能力或必要验证，定义被违反。 |
| Direct Scope | [术语合同](../plugins/sacha-orchestra/core/terminology-contract.md) | 当前执行者在同一目标内自主推进时采用的实施范围，由 Human 目标、明确约束和已批准 Spec（若有）界定；预计文件列表只有在 Human 或已批准 Spec 明确指定时才成为硬边界。 | 根 `AGENTS.md`、Workflow Contract | 若未有 Human 或已批准 Spec 的精确文件约束，就把预计文件列表作为硬边界，定义被违反。 |
...
| 显式发布文档目标 | [术语合同](../plugins/sacha-orchestra/core/terminology-contract.md) | Human 在显式 `document-project` 请求中直接提供的 project root 内 Markdown path，且文档类型为 `change-archive` 或 `system-guide`；不包括 Roadmap、Project Context、目录/root 或由 Project Integration 派生的目标。该术语只标识输入分类，授权、路由和写入动作仍归对应 Owner。 | `PLUGIN_DESIGN.md`、Intake Contract、Workflow Contract、document-project Skill、插件 README | 若未直接提供文件 path、类型不符或目标由配置派生却仍使用该术语，定义被违反。 |
