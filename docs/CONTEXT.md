...
| 能力加载策略 | [术语合同](../plugins/sacha-orchestra/core/terminology-contract.md) | 项目接入中已确认的能力绑定（Capability Binding）对规范技能的加载条件；只决定何时读取并采用技能，不表示已经获得授权、满足前置条件或执行动作。 | 工作流合同、setup-project 技能、Planner、Explore、Executor、Reviewer 技能、能力提供方接入指南 | 若策略直接授予动作、绕过技能前置条件，或者运行环境中的使用方无法取得四种策略的加载条件，定义失效。 |
