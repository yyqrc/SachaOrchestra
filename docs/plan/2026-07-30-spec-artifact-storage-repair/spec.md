# Spec Artifact 与 Spec storage root 统一修复
...
把持久任务权威统一为 `Spec Artifact`。Planner 只有在批准、breaking 或跨 context 恢复需要时才持久化，并在任务目录默认写入 `spec.md`。Project Integration 对外只暴露 `Spec storage root`；`Plan` 仅表示 lifecycle 中按需发生的规划活动或 `inline plan`。
...
| 持久的 `Plan` / 批准 Plan path | 持久的 `Spec` / 批准 Spec path |
| Planner 持久化文件名未规定或生成 `plan.md` | 任务目录默认 `spec.md` |
| `Plan storage root` / `- Plan：...` | `Spec storage root` / `- Spec：...` |
| `plan_storage` | `spec_storage` |
| `plan_root_kind` / `plan_root` | `spec_base_kind` / `spec_base` |
| `--plan-root-kind` / `--plan-root` | `--spec-base-kind` / `--spec-base` |
| `plan_root_unreachable` | `spec_base_unreachable` |

旧名必须从当前 Core、Skill、生成器和验证代码中删除。生成器只解析新 `Spec` 输出；旧输入不形成隐式配置，也没有 alias、fallback 或迁移提示。

## 实施约束

- Spec storage root 与 Project Documentation root 的 policy/write authorization 保持独立。
- Setup dry-run 不创建 Spec storage root；外部 Spec base 继续拒绝项目内路径与文件系统/盘符根，不可达时只给 `spec_base_unreachable` warning。
- 写入继续要求完整 planned delta、匹配的 SHA-256、managed marker 与旧文件 hash；并发变化拒绝，失败保持原子写/补偿恢复语义。
- Planner 的生产入口必须明确：需要持久化时，在 confirmed Spec storage root 下创建任务目录，并默认写入 `spec.md`；没有 confirmed root 时使用项目现有约定，不调用 Setup。
...
2. Setup Python API、CLI、JSON 结果与生成的 Project Integration 只使用 `spec_base*`、`--spec-base*`、Spec storage root / `- Spec：...`；Spec storage root 固定由 `<Spec base>/plan` 推导，旧 CLI 参数被 argparse 拒绝，旧输出不被解析为 root。
3. 行为测试证明 Spec base 与 Project Documentation root 独立，project-relative 与 external-absolute 均保留 portability；dry-run 不建目录，不可达 base 产生 `spec_base_unreachable`，盘符根被拒绝。
...
上述情况返回 Planner/Human；同 Scope 的实现缺陷、测试失败和 reference 漏改由 Executor 直接修复并重验。
