...
- Codex Adapter 只把 runtime-neutral route requirement 映射为非重叠的有序模型选择、一次原生调用和一次失败回退；把明确迁移映射为恰好一次 `create_thread`；把显式 Feedback 映射为原生 target 查询、唯一创建和 terminal join。
...
5. 新 task 接管完整剩余 lifecycle：Execute、subagent、Review/返修与 closeout。旧 task 在 create/handoff 成功后展示 target reference并结束，不等待 terminal return；Spec 不可达/未批准、仍有未决方案/授权、依赖未落盘历史、旧写入者未终止，或当前 task 已有等待其 terminal 的上游 return consumer 时不得迁移。显式 Feedback 创建的 repair target 属于后一种情况，保持 workflow owner 到 Source 消费根终态。
