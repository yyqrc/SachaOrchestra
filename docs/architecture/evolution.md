...
> 当前 release：`0.6.1` project-rules injection for project AGENTS
> 当前 source candidate：无
> 当前主线：Spec Artifact and owner-routed Feedback repair
> 发布边界：`0.6.1` setup-project 调用 provider `project.rules` skill 拿规则模板，通过 `--project-rules-file` 注入消费项目 AGENTS managed_block；marker hash + dry-run 保护刷新
