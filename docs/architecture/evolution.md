...
> 当前 release：`0.11.3` 正式 Reviewer 的 Sol 分档与有界调查
> 当前 source candidate：未开始
> 当前主线：主任务独占 Manager 并执行单层派发；Direct 出现改变执行方式的新事实时重新 Intake，Planner 冻结条件不足时先 Clarify
> 发布边界：`0.11.3` 不新增 Role、Gate、Artifact、Registry、Hook、MCP 或外部授权；模型命中与 Reviewer 调查边界的真实 Runtime 行为仍由全新主任务场景单独验证
...
| 版本线 | 当前事实 | 证据边界 |
| --- | --- | --- |
| `0.11.3` release | Codex 正式独立 Reviewer 使用 Sol：标准风险为 `medium`，已有 `critical` 事实才为 `xhigh`；Reviewer 调查按 Outcome 需要有界扩大并在必需检查裁决后停止 | 附注 tag 表示已发布源码；项目测试、Skill/Plugin validator 与 metadata coherence 只证明 source/static，安装只证明本机包版本。模型命中、调查扩大和停止行为仍需全新主任务中的真实 Runtime 场景验证 |
...
实现 Scope：Human 已批准主任务独占 Manager、单层派发、委派 Agent 协调请求、对应 Runtime 场景修正与插件开发术语治理；并基于真实路由与 Review 延迟强化逐单元模型路由、正式 Reviewer 的 Sol 风险分档和有界调查。
