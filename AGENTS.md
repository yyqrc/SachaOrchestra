...
- 普通发版执行者发现同一 Scope 已有仍有效的独立 Review，且 staged candidate、验收输入和证据边界未超出该 Review 时复用原结论；发版本身不触发重审。任一项变化时只审自原结论后的 staged delta 及其影响，按风险选择最低充分模型与推理强度，不因发布动作默认提高强度。
- 普通发版执行者先精确暂存当前待发布源码版本的候选内容，再并行运行风险对应验证、candidate coherence 与确有必要的增量 Review；通过后才把 Evolution 从 candidate 切为 release，并依次执行 commit、annotated tag、release coherence、原子 push 和远端核对。tag 建立前不得把 candidate 声明为当前 release，发布授权不得写入原实施 Scope。
...
- 普通发版实施收尾时若 Reviewer Gate 已有事实依据，当前 Owner 应完成必要 Review；发布阶段只核对 staged candidate 是否仍在该 Review 的 Scope、验收输入和证据边界内。发布脚本允许 candidate 外存在无关工作区改动，但 candidate path 暂存后又变化、存在冲突或 index 验证失败时必须停止。

普通发版先对当前待发布源码版本的精确暂存内容运行；`prepare` 从 Git index 导出隔离快照，验证不读取 candidate 外的 working/untracked 内容：

```powershell
python -B scripts/release.py prepare --version <version> --candidate-path <path> [--candidate-path <path> ...]
```

复用仍有效的 Review 或完成必要的增量 Review 后，维护者把 Evolution 从 candidate 切为 release 并精确暂存；再运行：
...
脚本不可用时，candidate/release 两阶段分别运行 metadata coherence：
...
`candidate` 在当前待发布源码版本阶段运行，只核对机器可解析部署身份和生产入口；`release` 在 commit、annotated tag 已建立且 Evolution 已人工切换为 release 后运行，并额外核对 annotated tag 精确指向 `HEAD`。该脚本不读取 README/Core/Adapter/Skill/Evolution 的说明文字。
