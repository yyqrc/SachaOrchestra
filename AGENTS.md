...
- Core、Adapter、Skill 和开发控制文档都必须遵守全局【表达要求】；除全局已经定义的正式标识外，不得另加语言豁免。
...
- 每条规则都必须明确回答四个问题：谁在什么情况下采取什么动作，以及最终产生什么结果或限制。可以使用自然句式，但不得省略其中任何一项。只保留会影响内容归属、直接消费者、流程判断、授权、安全、恢复、验收、证据或维护动作的限制，并删除背景铺垫、反向释义和同义补充。
- `description` 必须只回答“何时使用、何时不使用”；正文必须说明先查哪里、何时扩大范围、采取什么动作、交付什么以及何时停止；元数据提示词必须只提供自然入口，不得复述正文流程。
- Skill 正文必须默认按照“职责或功能、输入与首查、动作顺序、输出、停止与禁止边界”组织；Adapter 必须默认按照“实现的 Core 合同、Runtime 能力映射、调用参数、回退与恢复、Runtime 证据边界”组织。章节名称必须服从实际语义，不得为了套用结构而重复内容。
- 精简或压缩只能提高表达密度，不得用语义模糊换取更少字数。可以删除铺垫、常识、历史、同义重复和没有消费者的说明，但必须保留主体、触发或进入条件、动作及先后依赖、退出、停止和恢复条件、例外、Owner/Human 决策点、授权、安全、失败或未验证状态、Evidence、验收、Entry Condition 和 schema。改写后如果需要依赖上下文猜测、存在多种合理解释或无法证明语义等价，必须保留原文；如果会改变流程判断，则必须停止该部分并交给 Human 二次确认。
