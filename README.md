...
Sacha Orchestra 是跨项目、多智能体的工作流协调框架。本仓库包含平台中立的核心协议、Codex、Claude Code、Cursor 与 DeepSeek Harness 运行适配、四个 Runtime 共用的 Agent Plugin 源码与 marketplace 清单，以及独立构建的 DSH 可视化 companion plugin。
...
- [DSH 可视化 companion plugin](integrations/dsh/sacha-visualizer/README.md)独立构建和安装；它显示 Sacha 已记录转换及官方 Agent Teams roster/task DAG，不进入三个 marketplace，也不替代 Core、Adapter、Artifact 或 Runtime 证据。
