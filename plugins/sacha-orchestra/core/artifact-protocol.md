...
Spec Artifact 面向目标项目的 Human、Executor 和 Reviewer。必须在规划阶段完成本次实施所需的必要设计，将需求、澄清结果和技术决定落实为精确的修改说明，使执行者依据 Spec、项目规则及明确引用的项目资料，无需补充设计、比较方案或猜测意图即可实施。不得以最小方案、简洁或执行者自主性为由省略必要设计；说明精度不扩大设计范围，设计必要性与停止扩展条件遵循 [Workflow Contract 的运行原则](workflow-contract.md#2-运行原则)。这一标准统一覆盖可交接、可执行与自包含，不另设并列检查。移除 Sacha 上下文不得改变规格的技术含义。
...
Planner → 确定 Spec 中的已有事实 → 只接受项目规则、源码、配置、正式项目文档或已确认项目决定 → 其他来源不得单独定义项目约束或项目术语。必要的新设计由 Planner 根据这些事实与需求作出，明确标为拟议技术决定并随 Spec 提交审阅批准；不得伪装成已有事实，也不得因尚无现成实现而把设计留给执行者。
...
已有改动、验证结果和剩余工作等当前执行状态由既有 Execution Report 或恢复记录承载，接手所必需的记录须提供明确可达的 reference；不以另补一份“实施版”弥补 Spec 的方案缺失。
