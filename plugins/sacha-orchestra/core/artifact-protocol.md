...
| 2 | 范围 | 必需；分别说明本次包含与不包含的项目边界，以及预期改变、必须保留的行为与明确排除的做法，保留各自适用条件；不能只列文件或模块 |
| 3 | 项目事实与技术决定 | 必需；保存已确认事实、约束、技术决定与不变量，以及影响后续选择的决定理由和关键排除依据；不复制完整讨论过程 |
| 4 | 实施前提与依赖 | 按需；只记录项目环境、数据、工具和步骤依赖 |
| 5 | 实施方案 | 必需；说明项目位置、预期改动、顺序和完成结果 |
| 6 | 验收标准 | 必需；描述静态、构建、运行或人工可观察的项目结果，覆盖预期改变以及受本次方案影响的保留行为和禁止副作用，不写工作流执行分类 |
| 7 | 停用与回退约束 | 按需；项目确有停用、数据保护或回退要求时，说明现有控制方式、覆盖范围、必要清理和禁止副作用；不因格式要求新增开关、失败恢复或状态机 |
...
Spec 区分必须达到的结果、已冻结的技术决定与可替换的局部实现；具体手段只有已确认决定或必要项目约束支持时才写成必须保留，不因已有代码或旧报告推荐而固化。替换仍有效的冻结决定按 Workflow Contract 处理，不能以简化为由自行取消。
...
需求边界及必要理由写入上述对应内容，不另建同义清单或要求固定三段格式。没有已确认的排除项时不编造禁止事项；已有决定足以设计时不为填写格式重复提问。Spec 应使接手者无需原对话即可理解这些决定，不能只引用聊天、会话摘要或不可达记录来承载关键约束。

自包含允许接手者读取项目源码并选择可替换的局部实现，但不能要求其重新询问已确认决定、重新设计关键方案或猜测会改变行为与验收的边界。实施所需的技术决定写入 Spec；已有改动、验证结果和剩余工作等当前执行状态由既有 Execution Report 或恢复记录承载，接手所必需的记录须提供明确可达的 reference，不以另补一份“实施版”弥补 Spec 的方案缺失。
