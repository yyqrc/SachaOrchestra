...
| Direct | 当前主任务未接受 Sacha 生产生命周期，在现有目标、Scope、授权与验收内由当前上下文直接完成的路线；仍使用适用项目规则、领域能力和必要验证，语义转折形成新入口候选时重新判断。 |
| Direct Scope | 当前执行者在同一目标内自主推进时采用的实施范围，由 Human 目标、明确约束和已批准 Spec（若有）界定；预计文件列表只有在 Human 或已批准 Spec 明确指定时才成为硬边界。 |
