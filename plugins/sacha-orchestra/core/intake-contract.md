...
- Direct：目标、Scope、关键决定、授权与验收足够明确，当前上下文可安全完成，且没有下述实际规格交付或交接需求时，默认直接执行；跨文件、跨插件、规则或模型路由的行为变化本身不改变这一判断。
- 入口候选按 Planner Gate 事实处理：
  - 尚无 Planner Gate 事实时，持久 Owner、跨上下文恢复或正式编排会实质改变执行方式，且 Human 尚未选择是否进入 Sacha。
  - 已有 Planner Gate 事实时，仍有会改变实施路线的目标、验收、负责边界或关键方案待决定，且需要方案比较与冻结；或 Human 要求把完整可执行 Spec 交给后续实施/验收，或实际独立消费者、难回退的跨 Owner 决策、破坏性迁移需要规格交付。是否已经出现“持久化”或“落盘”字样不改变这个判断。
...
- 初次判断及 Direct 执行期间，主任务必须在继续形成实质方案、实施或持久化前检查语义转折。诊断演变为设计/修改、授权扩到新 Owner/平台，或新增 API 形态、Owner、回退/行为模式决策、Spec 消费者、跨上下文恢复需求，且这些事实会改变执行方式时，重新执行入口判断，只暂停依赖该决定的规划、派发、实施与持久化；完成入口决定前不得继续形成依赖该选择的单一路线实施方案。
- 元数据匹配入口候选时，先读取确认候选及说明选择影响所需的最小事实；候选成立且选择条件具备时及时提出入口选择，不以完整项目调查拖延提问。等待期间继续当前授权内、与流程选择无关且不会落实未批准方案的有界事实调查，按需加载相应只读 Domain Skill；该调查不得代替正式规划、冻结 Spec、派发或持久化，不得仅为 Sacha 路由提前加载生产合同和 Role。缺少答案只阻塞依赖该答案的部分；未答复不构成接受或拒绝。
- 同一目标或表面 Scope 名称未变，不得压过已改变的 Acceptance、风险、授权、Owner、实现边界或交付模型。没有第 3 节入口候选时保持 Direct。
- 主任务用第 3 节事实主动检查当前目标及自己准备推进的下一步，不以 Human 提及 Sacha、询问编排或授权启动实施为前提。候选判断不只针对本轮答复是否只读：准备提出原型或设计路线时，其中的关键决定、规格或协调需求同样参与判断，不会因被称为前置验证而移出当前目标。Human 已明确仅讨论独立的可行性问题、暂不推进该目标时，按这一窄目标判断；单纯技术解释或建议本身不构成候选。
- 候选成立且 Human 尚未接受或拒绝时，主任务在选择条件具备后主动推荐并询问一次是否进入 Sacha，按 Human Interaction Contract 说明新增能力、成本与执行影响；不得等 Human 问起才评估，也不得只给推荐后继续安排依赖决定的下游方案。若推荐没有第 3 节事实支持，应纠正推荐并保持 Direct，不因自己的推荐语句强造候选。Human 反问或尚不具备选择条件时，仍先按 Human Interaction Contract 解释，不机械追问。
