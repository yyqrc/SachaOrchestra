...
- 同一目标或表面 Scope 名称未变，不得压过已改变的 Acceptance、风险、授权、Owner、实现边界或交付模型。只有既无适用接受事实、也无第 3 节入口候选时保持 Direct。
...
- 候选成立且 Human 尚未接受或拒绝时，主任务在选择条件具备后必须主动推荐并询问一次是否进入 Sacha，按 Human Interaction Contract 说明新增能力、成本与执行影响；不得等 Human 问起才评估，也不得只给推荐后继续安排依赖决定的下游方案。若推荐没有第 3 节事实支持，应纠正推荐并保持 Direct，不因自己的推荐语句强造候选。Human 反问或尚不具备选择条件时，仍先按 Human Interaction Contract 解释，不机械追问。
