...
主任务在既有 Scope 与授权内只有一个有界委派 Agent 时可直接管理，并负责同等的评估、等待、取消和证据核对；仅拥有相应写入授权时才集成候选补丁。Explore 的研究委派 Agent 保持只读；Role 与裁决仍由主任务持有。

## 2. 评估、拆分、依赖与就绪判定

Manager 先按目标、Scope、授权、完成条件和输入事实拆分候选单元，建立依赖图和依赖波次；原授权与单写入者约束继续适用。

主任务在每次首次创建前必须对每个候选单元完成同一套不依赖 Runtime 的评估：目标类型、风险、难度/歧义、依赖/上下文、自包含性、可验证性、返工成本和独立性；必要时补充权限、破坏性变更、持久数据与独立 Reviewer 事实，并形成供当前 Runtime Adapter 消费的逐单元路由要求。未完成评估或无法形成路由要求时不得调用原生创建工具。Manager 为多个候选单元负责，主任务为单个委派 Agent 负责。
