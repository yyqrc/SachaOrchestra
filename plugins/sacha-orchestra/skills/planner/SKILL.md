...
3. 读取已落盘决定和相关项目 `CONTEXT.md`，按 Workflow Contract 判断目标结果、Scope/Non-goals、验收和会改变方案的 Human 决定是否满足冻结条件。存在未收口项时必须调用 `$sacha-orchestra:explore`；Explore 返回且冻结条件满足前不得进入 Spec 持久化。代码、项目规则或 Skill 可确认的事实先自行调查。
