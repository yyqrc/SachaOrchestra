...
7. 按 Artifact Protocol 将需求边界、技术决定和必要理由落入 Spec，区分必需结果与可替换的局部实现；修订时同步替换依赖已改变前提的旧方案和验收。回对适用条件、关键保留或排除理由，以及验收能否发现违反这些要求的实现。只提供项目规则、项目事实和 Spec，核对不了解 Sacha、看不到原对话的 Executor 与 Reviewer 能否据此继续；只写“禁止”而丢失影响取舍的原因、仅在聊天中确认或只列改动文件均不算完成。
