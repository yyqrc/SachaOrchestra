...
4. 当前 context 可恢复时用 inline plan；只有批准、breaking 或跨 context 恢复需要时写 Spec Artifact。
   持久化使用已确认的 Spec storage，默认任务文件为 `spec.md`；没有 confirmed storage 时遵循项目现有约定，不调用 Setup。
