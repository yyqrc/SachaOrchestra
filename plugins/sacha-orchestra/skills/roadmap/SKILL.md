...
1. 先读取并遵守 [Workflow Contract 的运行原则](../../core/workflow-contract.md#2-运行原则)，从项目来源和 Human 决定确定目标与完成形态、当前状态、适用范围、排除范围和稳定约束；Roadmap 不自行定义项目事实或改变 Scope。最小方案原则同样适用于阶段与工作面的选择，章节要求不得用于推导额外工作、边界调查或配套机制。
