...
2. 对自动匹配的入口候选，只读取确认当前目标及语义转折是否形成候选所需的最小事实，并判断 Planner Gate 事实是否成立。完整 Spec 已明确供后续实施或验收使用时构成候选，不要求输入同时写出“持久化”或“落盘”；目标、Scope、验收、风险、授权或交付模型的实质变化也可以形成新候选，reference、日志和进度变化沿用原判断。
