...
1. 建立当前 Baseline；存在 Spec 时按 [Artifact Protocol](../../core/artifact-protocol.md#21-spec-artifact) 检查差异与证据是否满足受影响的需求边界，不能只核对新增功能而遗漏保留行为和禁止副作用。只重跑可能改变裁决的验证。
2. 按独立目的核对每块改动的必要性：解决哪项当前需求或真实问题，现有实现为何不足，是否有更小的可行改法；新增代码、配置、规则和测试设施均适用。对缺少依据的新增指出具体位置、引入成本及删除或收缩建议，按 Assurance Contract 裁决。对变更接口追踪声明、实现和当前消费者，核对成功、失败、取消、所有权、清理、恢复、持久化和边界条件。
