...
2. 按顺序读取 Assurance Contract、当前 Scope 与 Baseline、精确 diff/文件集、会改变 Outcome 的裁决问题、调用方已提供的原始证据，以及受影响的唯一 Owner 和直接消费者。上述输入足以裁决时直接检查，不为恢复背景或追求完整重新调查历史；Project Integration 有目标 Skill 的已确认加载条目时，按 [Workflow Contract](../../core/workflow-contract.md) 的技能加载策略决定是否加载，加载后完整读取正文并另行核对前置、具体副作用、授权和会改变裁决的验证入口。策略不允许或缺少条目、可见 Skill 时，回退 AGENTS、Domain Skill 或原生路线并保留未验证项。
3. 核对来源独立性。独立 Reviewer 使用未参与当前方案和实现的上下文；参与者只提交自检结果。

## 动作顺序

1. 建立当前 Baseline；存在 Spec 时按 [Artifact Protocol 的 Spec 标准](../../core/artifact-protocol.md#21-spec-artifact)核对实际修改与必要设计是否一致、是否存在执行者未经确认的设计补充或替换，并检查受影响的需求与验收。规格缺失时指出具体位置和必须重新设计的内容，按既有路由返回，不由 Reviewer 补设计，也不另建可交接、可执行或自包含检查清单。只重跑可能改变裁决的验证。
2. 按独立目的核对每块改动的必要性：解决哪项当前需求或真实问题，现有实现为何不足，是否有更小的可行改法；新增代码、配置、规则和测试设施均适用。对缺少依据的新增指出具体位置、引入成本及删除或收缩建议，按 Assurance Contract 裁决。对变更接口追踪声明、实现和当前消费者，只检查本次实际改变的行为、直接受影响的保留行为及有具体事实依据的问题；必须遵守 [Workflow Contract 的运行原则](../../core/workflow-contract.md#2-运行原则)，不得按类别补齐边界检查，或仅因缺少假设场景的保护机制而要求返修。
