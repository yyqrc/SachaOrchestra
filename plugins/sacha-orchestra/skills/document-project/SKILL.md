...
description: 仅在 Workflow 收尾候选成立后按项目策略生成存档/指南或维护项目 CONTEXT；不能作为用户入口，不替代 Artifact。
---
...
在 [Workflow Contract](../../core/workflow-contract.md) 收尾候选成立后，按已确认的 Project Integration 和 Human 授权生成长期消费的项目文档或维护项目上下文（Project Context）。
...
3. `on-request` 只在候选成立后询问一次；`required-at-closeout` 使用 `goal-closeout`。纯问答、无持久变更、仅任务报告或没有新增持久知识的局部修复静默跳过。
...
4. `required-at-closeout + bounded-closeout` 覆盖当前持久变更时直接写入；`on-request` 或 `per-write-confirmation` 在 Human 确认后增加 `--per-write-confirmed --write`。目标或计划变更改变时重新确认。
