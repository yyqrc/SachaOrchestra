...
1. Human 显式要求对齐、解释、调查或讨论，或其当前表达不足以证明其掌握项目背景、实现机制或术语时，先进入 `brainstorm` 或 `survey`；不得因其使用了专业词、提出候选或发起任务就直接进入选择题。
2. 根据当前输入组合以下工作意图：
   - `brainstorm`：想法还模糊时，先同步目标结果、Non-goals 和主要约束；存在实质不同的候选时，先解释差异与未知，再在具备选择条件后比较取舍并给出推荐。
   - `survey`：方向已有但现状、内部先例或外部方案不清时，先用代码、项目资料、可用 Skill 或必要的一手资料形成可比较事实并解释因果，再判断是否仍有 Human 取舍。
...
3. 维护只覆盖实质分支的有界挑战图，标记可问项、上游依赖、待查事实、排除依据和未决项；按依赖顺序处理。
4. 向 Human 提问、解释或报告进度前读取 [Human Interaction Contract](../../core/human-interaction-contract.md)。追问、解释、调查、纠正和决定在同一对话循环中互相触发，并按以下语义更新挑战图：
   - Clarify → 首次解释或 Human 表示不了解 → 先用不依赖内部术语的概要说明当前状态、因果、已知/未知与真正的决定边界；Human 继续追问时再展开机制、数据流和证据 reference → 解释完成前不要求 Human 选择，也不询问 Human 是否同意 Agent 的分析。
...
5. 沿用[术语合同](../../core/terminology-contract.md)的主任务、委派 Agent 与协调请求。必要事实不可低成本取得时，向主任务请求一个有界只读研究委派 Agent，给出问题、查询范围、预期证据、停止条件和输入。主任务出现多个候选问题、依赖或恢复协调时，按 [Coordination Contract](../../core/coordination-contract.md) 调用 Manager 并消费其事实、冲突、未验证项和证据 reference；Clarify 委派 Agent 只返回研究结果或协调请求。
6. 已知后续会形成 Spec 或需要恢复时，第一个影响方案的决定确认后写入项目既有决定载体；多个未决项、分支打断或跨上下文风险出现时建立澄清锚点。无项目约定时在 Spec 任务目录使用最小 `decisions.md`。
7. 锚点只保存原始目标、已确认决定、当前关注点、阻塞性未决项、暂存思路、reference 和最小可恢复边界。恢复后先读锚点与当前证据，再重建挑战图。
8. 新思路用于解决当前未决项、增加阻塞性未决项或暂存为非阻塞候选。改变目标、Scope 或验收的新思路返回 Planner/Human。
9. 需供恢复或 Spec 消费的术语写入决定记录。具备任务外消费者或稳定接口的项目术语记录为项目上下文（`project-context`）候选，包含定义、排除含义、证据、边界、消费者与 `Unknown`。
10. 退出前扫描相关挑战面：影响方案的重要分支均已解决、明确暂缓、路由调查或保留为阻塞项；决定具备事实依据且关键歧义已消除。
