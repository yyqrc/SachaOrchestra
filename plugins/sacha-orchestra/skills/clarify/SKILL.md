...
拥有 Planner 前的有界澄清节点：补齐会改变方案的事实与 Human 决定，再把事实、决定、冲突和未决项交回 invoking owner。Scope、授权、实施和裁决由对应 Role/Human 拥有。

## 输入与首查

1. 入口为 Human 显式调用，或 active Planner 发现目标结果、Scope/Non-goals、验收或实质决定未收口后的显式路由。
...
4. 必要事实不可低成本取得时，向当前 owner 请求一个有界只读 helper，给出问题、查询范围、预期证据、停止条件和输入。出现多个候选问题、依赖或恢复协调时，按 [Coordination Contract](../../core/coordination-contract.md) 调用 Manager，并消费其事实、冲突、未验证项和 evidence reference。
5. 已知后续会形成 Spec 或需要恢复时，第一个影响方案的决定确认后写入项目既有决定载体；多个未决项、分支打断或跨 context 风险出现时建立澄清锚点。无项目约定时在 Spec task directory 使用最小 `decisions.md`。
6. 锚点只保存原始目标、已确认决定、当前关注点、阻塞性未决项、暂存思路、reference 和最小可恢复 frontier。恢复后先读锚点与当前证据，再重建挑战图。
7. 新思路用于解决当前未决项、增加阻塞性未决项或暂存为非阻塞候选。改变目标、Scope 或验收的新思路返回 Planner/Human。
8. 需供恢复或 Spec 消费的术语写入决定记录。具备任务外消费者或稳定接口的项目术语记录为 project-context 候选，包含定义、排除含义、证据、边界、消费者与 Unknown。
9. 退出前扫描相关挑战面：影响方案的重要分支均已解决、明确暂缓、路由调查或保留为阻塞项；决定具备事实依据且关键歧义已消除。

## 输出

- 向 invoking owner 返回已核实事实、Human 决定、冲突、阻塞性未决项、未验证项、evidence reference 和退出判断。
- active Planner 负责判断这些结果是否足以冻结 Spec。
