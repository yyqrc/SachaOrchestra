...
拥有 Planner 前的有界澄清节点：补齐会改变方案的事实与 Human 决定，再把事实、决定、冲突和未决项交回调用 Owner。Scope、授权、实施和裁决由对应 Role/Human 拥有。
...
4. 必要事实不可低成本取得时，向当前 Owner 请求一个有界只读辅助 Agent，给出问题、查询范围、预期证据、停止条件和输入。出现多个候选问题、依赖或恢复协调时，按 [Coordination Contract](../../core/coordination-contract.md) 调用 Manager，并消费其事实、冲突、未验证项和证据 reference。
...
- 向调用 Owner 返回已核实事实、Human 决定、冲突、阻塞性未决项、未验证项、证据 reference 和退出判断。
