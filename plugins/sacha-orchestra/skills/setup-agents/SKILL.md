...
3. 写入前重读两个 preimage，全部临时 TOML 校验后才原子替换。写后回读内容/hash、必填字段和两个 Agent identity；任一失败则整批补偿恢复。
4. 返回 action、transaction、目标/hash、delta 和 warning。文件存在或 parse 成功不代表 Runtime 已发现。
