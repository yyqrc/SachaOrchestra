...
主任务消费 Coordination 已确定的本地执行、复用或新建决定；需要新建时使用 `Agent`，复用使用宿主可用的继续入口。已决定并行且 `parallel_expected` 成立时，在消费完成结果前启动相应实例。单个有界委派同样按第 3.4 节选择只读或写入路线，不因 Adapter 调用另开 Manager Gate。委派 Agent 满足条件时返回协调请求。共享工作树的同一文件/输出由集成 Owner 串行处理；隔离补丁/候选实现可使用并行 `Agent` 上下文。
