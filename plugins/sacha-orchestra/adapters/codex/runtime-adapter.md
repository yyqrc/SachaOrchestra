...
提炼术语、Intake、Role、Gate、readiness、Manager 职责、批准路由和 Artifact 结构由对应 Core/Skill 拥有。本 Adapter 消费已确定的 Human 交互动作与路由要求；只有主任务执行子代理调度，委派 Agent 的有界消息按第 2.2 节。Direct/当前上下文保持当前模型与 Owner。
...
| 运行中补充或改向 | `send_message` | 主任务在同一标识、Owner 和连续目标内使用；独立工作单元按 Coordination 第 2 节新建，不借补充消息塞入旧代理 |
| 委派 Agent 交换事实或返回协调请求 | `send_message` | 只向已知主任务或同级发送 Coordination 允许的信息，不用消息分配工作或要求对方启动、继续、中断 |
| 已结束回合后继续 | `followup_task` | 仅主任务；满足 Coordination 第 2 节复用条件时继续原标识，否则按第 3 节新建 |
| 等待 | `wait_agent` | 仅主任务在真实依赖处等待；通知或超时不等于进程终止 |
| 中断当前回合 | `interrupt_agent` | 仅主任务在取消、失活或继续会增险时使用；代理仍可接收后续消息 |
| 核对与恢复标识 | `list_agents` | 仅主任务有界查询原目标；无唯一标识时停止，不创建替代写入者 |
| 释放已完成目标 | 无独立关闭工具 | 消费结果并保留原始标识与证据 |

`followup_task` 继续原代理并保留其历史上下文；首次创建的 `fork_turns="none"` 只控制父任务历史的继承，不会在复用时清空子代理历史。主任务后续信息通过消息显式交付，不假定自动同步。新建按第 3 节使用自足输入。

容量以当前宿主工具说明和实际状态为准，区分并发槽位、主任务是否占位与累计创建限制，不硬编码数量或推断未声明的累计上限。达到容量时按真实依赖等待，或由主任务处理适合的短小动作；不得为绕过容量把独立工作塞入旧代理，也不假定回合结束或中断必然释放槽位。

中断成功只证明当前回合被中断。接管写入前还须从代理状态和相关进程、任务或工具结果确认写入已停止；若底层操作仍运行或状态未知，只暂停有双写风险的部分，不把通知记录成完全取消。

### 2.3 Codex Role 调用面

主任务按第 3 节为 Planner、Reviewer、Executor、Explore 研究和普通工作单元组装首次创建参数；Role 作为评估输入，协作界面只决定传输编码。Manager 在主任务内运行，不是委派 Agent。委派 Agent 只使用第 2.2 节允许的消息，不创建、继续驱动或中断其他代理；所需调度返回主任务。
...
- 等待时长在当前工具允许范围内兼顾宿主沟通要求与预计结果时间；通知可提前唤醒时，不为捕获完成而反复短等。没有可推进工作时沿用阻塞等待，不用定时消息催报或通过中断再继续探测进度。
- 进入等待时，按工作包预期耗时、最近实际活动和任务截止时间确定下一次活性核对点；它可以跨越多次 `wait_agent` 超时，不另建定时任务或固定频率催报。到点仍无完成回传，或通知缺少会改变下一步的状态时，才对已知目标有界调用 `list_agents`。
- 等待超时或上下文压缩都不证明失活。状态显示仍有有效进展时更新核对点并继续等待；已结束但缺结果时先消费可达的原生结果，仍缺失才一次请求最小结果/阻塞摘要。状态不明或无进展时，先核对可达工具/进程活动，再决定补问、恢复或取消；同一未知状态只延长等待间隔或按截止条件处理，不反复催报，也不以超时创建替代代理。取消与写入接管仍遵守第 2.2 节。
- Human 进度使用已知的新事实；需要沟通不自动触发代理状态查询，不为生成进度消息重读相同记录。
...
- 先核对精确型号/强度要求的适用对象与阶段，再优先消费。创建或迁移主任务时指定的型号/强度默认只约束该主任务；只有用户明确指定子代理、相应工作单元或整个任务统一使用时，才对子代理采用 human_exact。交接文本携带主任务的型号要求，不自动扩大其适用对象；未覆盖当前子代理时，按该工作单元采用自动路线。
- 精确要求适用但当前工具不支持时停止，不能自动换档。规划阶段的 Astra high/xhigh 即使写入批准范围，也不延续到实施或验收；只有用户针对该阶段另行明确指定时才走 human_exact，否则重新采用自动路线。
...
| route_id | 判断 | 请求字段 |
| --- | --- | --- |
| `human_exact` | 用户或批准范围明确对当前子代理及阶段精确指定，且满足 A 节对象与阶段适用性 | 原样使用当前工具支持的型号与强度；额外参数不支持时停止 |
...
执行中发现冻结前提不成立时，返回受影响的方案问题，不以困难、失败或反复尝试自动提高强度。满足 Coordination 第 2 节复用条件时保留原模型，新建时按本节路由，不假定跨模型或新代理继承原缓存。使用实际输入、缓存、输出、耗时与返工并核对宿主计费口径；遥测不足时保留不确定性，不宣称切换更省，不以单价、累计 token 或固定阈值自动切换，也不要求每次重新跑基准。
