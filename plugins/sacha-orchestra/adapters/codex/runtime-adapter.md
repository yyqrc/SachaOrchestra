...
- 等待时长在当前工具允许范围内兼顾宿主沟通要求与预计结果时间；没有可推进工作时，不用短间隔唤醒反复判断同一状态。
- 超时不保证返回完整状态，也不证明失活。没有新结果且依赖未变时沿用已知标识继续等待；只有通知或返回缺少会改变下一步的状态、标识或停止证据时，才有界调用 `list_agents`，不在每次超时后例行查询。
