...
| 运行中补充或改向 | `send_message` | 主任务在同一标识、Owner 和连续目标内使用；新独立目标先按 Coordination 重评复用与就绪 |
| 委派 Agent 交换事实或返回协调请求 | `send_message` | 只向已知主任务或同级发送 Coordination 允许的信息，不用消息分配工作或要求对方启动、继续、中断 |
| 已结束回合后继续 | `followup_task` | 仅主任务；连续目标复用原标识，独立后续工作须先按 Coordination 与第 3 节评估，不绕过范围与就绪判断 |
...
执行中发现冻结前提不成立时，返回受影响的方案问题，不以困难、失败或反复尝试自动提高强度。按 Coordination 的剩余成本判断复用时保留原模型，新建时按本节路由，不假定跨模型或新代理继承原缓存。使用实际输入、缓存、输出、耗时与返工并核对宿主计费口径；遥测不足时保留不确定性，不宣称切换更省，不以单价、累计 token 或固定阈值自动切换，也不要求每次重新跑基准。
