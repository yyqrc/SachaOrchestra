...
describe('visual event replay', () => {
  ...
  it('folds latest phase, gate, wave, delegation, review, and evidence values', () => {
    const replay = recordedVisualEvents([
      call(0, 'p1', { event_type: 'phase', summary: '入口', phase: 'intake', phase_state: 'entered', scope_revision: 'r1' }), result(1, 'p1'),
      ...
      call(8, 'e1', { event_type: 'evidence', summary: 'Runtime 未验证', evidence_layer: 'runtime', evidence_status: 'unverified', scope_revision: 'r1' }), result(9, 'e1'),
      call(10, 'r1', { event_type: 'review', summary: '需要补证', outcome: 'needs_evidence', scope_revision: 'r1' }), result(11, 'r1'),
    ])
  ...
  it('retains old and unassociated results only in history, including late results', () => {
    const events = [
      call(0, 'p1', { event_type: 'phase', summary: 'r1', phase: 'reviewer', phase_state: 'entered', scope_revision: 'r1' }), result(1, 'p1'),
      call(2, 'old', { event_type: 'review', summary: '旧结果', outcome: 'accepted', scope_revision: 'r1' }), result(3, 'old'),
      call(4, 'late', { event_type: 'evidence', summary: '旧证据', evidence_layer: 'source', evidence_status: 'verified', scope_revision: 'r1' }),
      call(5, 'p2', { event_type: 'phase', summary: 'r2', phase: 'executor', phase_state: 'entered', scope_revision: 'r2' }), result(6, 'p2'),
      result(7, 'late'),
      call(8, 'legacy', { event_type: 'review', summary: '未关联', outcome: 'accepted' }), result(9, 'legacy'),
    ]
    const replay = recordedVisualEvents(events)
    expect(replay.warnings).toEqual([])
    expect(replay.events).toHaveLength(5)
    expect(foldVisualState(replay.events)).toMatchObject({ evidence: {} })
    expect(foldVisualState(replay.events).review).toBeUndefined()
    events.push(call(10, 'current', { event_type: 'review', summary: '当前结果', outcome: 'accepted', scope_revision: 'r2' }), result(11, 'current'))
    expect(foldVisualState(recordedVisualEvents(events).events).review).toMatchObject({ outcome: 'accepted', scopeRevision: 'r2' })
  })
})
